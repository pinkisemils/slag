...
impl SlackReceiver {
    ...
    fn notify_of_disconnect(&mut self) {
        let status_messages: Vec<PrivMsg> = self.cfg.channels.keys()
            .map(|chan| {
                PrivMsg{
                    chan: chan.clone(),
...
impl SlackSender {
    ...
    fn try_slack_msg_from_priv(cfg: &SlackCfg, pmsg: PrivMsg) -> Option<SlackPayload> {
        info!("do I ever?");
        for (key, val) in cfg.channels.iter() {
            info!("I gotz irc chan({}) -> slack chan({})", key, val);
        }
        let out_chan = cfg.channels.get(&pmsg.chan)?;
        info!("channel found -> {}", out_chan);
